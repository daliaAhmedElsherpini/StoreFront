...
const updateUserValidation = [
  ...
  check('email')
    ...
    .custom(async (email: string, { req }: any) => {
      // req here its type is any form the express validator package
      const existingUser = await model.where(
        `email = '${email}' AND id != ${req.params.id}`
      )
  ...
  check('phone')
    ...
    .custom(async (phone: string, { req }: any) => {
      // req here its type is any form the express validator package
      const existingUser = await model.where(
        `phone = '${phone}' AND id != ${req.params.id}`
      )
